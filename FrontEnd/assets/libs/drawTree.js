function DrawTree() {
  ...
  var svg = d3v4.select("#tree").select("svg")
  ...
  obj.draw = function(data) {
    ...
    // var root = d3v4.hierarchy(parseNewick(life), function(d) { return d.branchset; })
    //     .sum(function(d) { return d.branchset ? 0 : 1; })
    //     .sort(function(a, b) { return (a.value - b.value) || d3v4.ascending(a.data.length, b.data.length); });
    ...
    var input = d3v4.select("#show-length input").on("change", changed),
    ...
    chart.append("g")
      ...
      .enter().append("text")
        ...
        .text(function(d) { return d.data.label.replace(/_/g, " "); })
        .on("mouseover", mouseovered(true))
        .on("mouseout", mouseovered(false));

    function changed() {
      clearTimeout(timeout);
      var t = d3v4.transition().duration(750);
    ...
    function mouseovered(active) {
      return function(d) {
        d3v4.select(this).classed("label--active", active);
        d3v4.select(d.linkExtensionNode).classed("link-extension--active", active).each(moveToFront);
        do d3v4.select(d.linkNode).classed("link--active", active).each(moveToFront); while (d = d.parent);
      };
    }
  ...
  function maxLength(d) {
    return d.data.value + (d.children ? d3v4.max(d.children, maxLength) : 0);
  }
  ...
  // Like d3v4.svg.diagonal.radial, but with square corners.
